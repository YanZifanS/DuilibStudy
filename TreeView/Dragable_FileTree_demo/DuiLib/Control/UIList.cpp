...
int __cdecl CListBodyUI::ItemComareFunc(void *pvlocale, const void *item1, const void *item2)
{
	...
	if (!item1 || !item2)
		return 0;
	CListTextElementUI *pControl1 = *(CListTextElementUI **)item1;
	CListTextElementUI *pControl2 = *(CListTextElementUI **)item2;

	return lstrcmp(pControl1->GetText(i),pControl2->GetText(i));
	//return pThis->ItemComareFunc(item1, item2);
}


BOOL CListBodyUI::SortItems(int iIndex)
{

	qsort_s(m_items.GetData(), m_items.GetSize(), sizeof(CControlUI*), CListBodyUI::ItemComareFunc, &iIndex);	
...
CListTextElementUI::~CListTextElementUI()
{
	...
	//icon
	HICON hIcon;
	for( int it = 0; it < m_aIcon.GetSize(); it++ ) {
		 hIcon = static_cast<HICON>(m_aIcon[it]);
		if( hIcon)
			DestroyIcon(hIcon);
	}
	m_aIcon.Empty();
...
{
	HICON hIcon = static_cast<HICON>(m_aIcon.GetAt(iIndex));
	if( hIcon ) 
		return hIcon;
	return NULL;
}
void CListTextElementUI::SetIcon(int iIndex, HICON hIcon)
{
	...
	if( iIndex < 0 || iIndex >= pInfo->nColumns )
		return;
	while( m_aIcon.GetSize() < pInfo->nColumns )
	{ 
		m_aIcon.Add(NULL);
	}
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
		...
		if (hIcon)
		{
			DrawIconEx(hDC,pInfo->rcColumn[i].left+2, m_rcItem.top+2,hIcon,16,16,0,NULL,DI_NORMAL);
			
			rcItem.left += pInfo->rcTextPadding.left + 18;
		}
		else
		{
