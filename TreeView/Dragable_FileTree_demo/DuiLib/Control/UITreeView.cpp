...
namespace DuiLib
{
	...
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		if(_ParentNode)
		{
			...
			pDottedLine->SetFixedWidth(_ParentNode->GetDottedLine()->GetFixedWidth()+18);
	...
	CTreeNodeUI* CTreeNodeUI::CalLocation( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		_pTreeNodeUI->GetDottedLine()->SetFixedWidth(pDottedLine->GetFixedWidth()+18);
	...
	bool CTreeViewUI::OnFolderChanged( void* param )
	{
		...
		if(pMsg->sType == _T("selectchanged"))
		{
			...
			if (!pFolder->GetCheck())
			{
				m_pManager->SendNotify(pItem,_T("itemexpand"));
			}
